...
/*
 * Extract a value of type vector<T> from its string representation
 * contained in strVal. The VectorLike parameter used to select the
 * correct  * specialization of ExtractValue depending on the value
 * traits of T. VectorLike traits use operator>> multiple times to
 * assign the value from strVal.
 */
template<typename T> void
ExtractValue(T &destVal, const std::string& strVal, VectorLike vl)
{
	static_cast<void>(vl); // Avoid warning about unused vl
	std::istringstream is(strVal);
	std::string token;
	...
	while(std::getline(is, token, ',')) {
		...
		ExtractValue(value, token, ValueLike());
		destVal.push_back(value);
	}

	if(is.fail() && !is.eof())
		throw(ArgParseException("Couldn't read argument vector from string '" + strVal + "'"));
}
