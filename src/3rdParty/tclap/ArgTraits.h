...
/**
 * A value like argument value type is a value that can be set using
 * operator>> with comma separated values.
 */
struct VectorLike {
	virtual ~VectorLike() {}
};
...
/**
 * A class can inherit from this object to make it have vector like
 * traits. This is a compile time thing and does not add any overhead
 * to the inherenting class.
 */
struct VectorLikeTrait {
	typedef VectorLike ValueCategory;
	virtual ~VectorLikeTrait() {}
};
